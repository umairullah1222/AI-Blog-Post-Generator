...
import { generateBlogPostStream, generateBlogPostGrounded, generatePostImage, analyzeContent, analyzeSeo } from '../services/geminiService.ts';
...
import type { GenerateOptions, WordPressCredentials, HistoryItem, AutomationJob, AutomationSettings, ResearchResult, GroundingSource, User, UserPreferences, SeoAnalysisResult, View } from '../types.ts';

interface AppContextType {
    // Auth & User
    currentUser: User | null;
    handleLogin: (credentials: Pick<User, 'email' | 'password'>) => void;
    handleSignup: (newUser: Pick<User, 'username' | 'email' | 'password'>) => void;
    handleLogout: () => void;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // --- Effects for initial loading from localStorage ---
    useEffect(() => {
    ...
    const handleLogin = useCallback((credentials: Pick<User, 'email' | 'password'>) => {
    ...
    const handleSignup = useCallback((newUser: Pick<User, 'username' | 'email' | 'password'>) => {
        const result = signup(newUser as User);
        if (result.success && result.user) {
            setCurrentUser(result.user);
            setAuthSuccess(result.message);
            setAuthError(null);
    ...
    const handleLogout = useCallback(() => {
        logout();
        setCurrentUser(null);
        setActiveView('home'); // Redirect to home on logout
    }, []);
    ...
    const handleUpdateProfilePicture = useCallback(async (imageBase64: string): Promise<{ success: boolean; message: string; }> => {
        ...
        try {
            const updatedUser = authUpdateProfilePicture(currentUser.username, imageBase64);
            if (updatedUser) {
        ...
        for (let i = 0; i < jobs.length; i++) {
            ...
            if (settings.autoPublish) {
                ...
                try {
                 ...
                 addToHistory({ topic: currentJob.topic, content, image });
    ...
    const contextValue: AppContextType = {
        currentUser, handleLogin, handleSignup, handleLogout, authError, authSuccess, clearAuthStatus, handleUpdateProfilePicture, isAuthModalOpen, setIsAuthModalOpen,
